...
export default class VideoSubscriptionPlan extends Component {
    ...
    componentDidMount = () => {
        this.props.navigation.addListener('focus', () => {
            if (config.device_type = 'ios') {
                setTimeout(() => {
                    this.getVideoSubscription();
                    this.setDetailsVideo();
                }, 600);
            } else {
                this.getVideoSubscription();
                this.setDetailsVideo();
    ...
    takeSubscriptionPlan = async (amount, no_of_video, plan_id) => {
        ...
        this.setState({ amount: amount, user_id: user_details.user_id });
        let url = config.baseURL + "takeVideoSubscription.php";
        consolepro.consolelog('url', url)
        let data = new FormData();
        data.append('user_id', user_details.user_id)
        data.append('amount', amount)
        data.append('no_of_video', no_of_video)
        data.append('plan_id', plan_id)
        apifuntion.postApi(url, data).then((obj) => {
            if (obj.success == 'true') {
                consolepro.consolelog('video_subscription_id', obj.video_subscription_id)
                this.setState({ video_subscription_id: obj.video_subscription_id })
                localStorage.setItemObject('video_subscription_id', obj.video_subscription_id)
        ...
        if (webViewState.loading == false) {
            ...
            if (typeof (t) != null) {
                ...
                if (t == 'payment_success_final.php') {
                    ...
                    this.video_payment_status_update(payment_id)
    ...
    getVideoSubscription = async () => {
        consolepro.consolelog('getVideoSubscription');
        let user_details = await localStorage.getItemObject('user_arr');
        consolepro.consolelog('user_details_ba', user_details);
        let url = config.baseURL + "getVideoSubscription.php?user_id=" + user_details.user_id;
        ...
        return (
            <TouchableOpacity activeOpacity={1} onPress={() => { }}
                ...
                <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <Text style={{ color: Colors.purewhite, fontFamily: Font.regular, fontSize: mobileW * 3.6 / 100, }} >(Monthly)</Text>
                    <Text style={{ color: Colors.purewhite, fontFamily: Font.bold, fontSize: mobileW * 3.6 / 100, marginTop: 5 }}>Play {item[5]} Videos</Text>
                    <TouchableOpacity activeOpacity={1} onPress={() => { this.takeSubscriptionPlan(item[2], item[5], item[0]); console.log('amount2', item[0]); console.log('no_of_video', item[5]) }}
                        style={{ backgroundColor: Colors.theme_color, height: mobileH * 5 / 100, width: mobileW * 32 / 100, alignSelf: 'center', borderRadius: 10, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', borderColor: Colors.purewhite, borderWidth: 1, marginTop: 10 }}>
                        ...
                        <View style={{ flex: 1, backgroundColor: 'white' }}>
                            {
                                ...
                                <WebView
                                    source={{ uri: config.baseURL + 'stripe_payment/payment_url.php?user_id=' + this.state.user_id + '&order_id=' + this.state.video_subscription_id + '&descriptor_suffix=genius_genie&transfer_user_id=0&transfer_amount=0&amount=' + this.state.amount }}
