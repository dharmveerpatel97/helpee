	...
	getMyInboxAllData = async () => {
		...
		if (userdata != null) {
			...
			queryUpdate.on('child_changed', (data) => {
				...
				var question_status = data.val().question_status
				var question_id = data.val().question_id
				var onlineStatus = data.val().onlineStatus
				...
				if (question_id != 0) {
					for (var i = 0; i < FirebaseInboxJson.length; i++) {
						if (FirebaseInboxJson[i].question_id == question_id) {
							...
							FirebaseInboxJson[i].question_status = question_status;
