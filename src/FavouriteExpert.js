...
export default class FavouriteExpert extends Component {
    constructor(props) {
        super(props)
        this.state = {
            content_2: '',
    ...
    SearchFilterFunction = (text) => {
        ...
        if (this.state.onCross == false) {
            ...
            consolepro.consolelog('test1', this.state.favourite_arr)
            let data1 = this.state.favourite_arr1
            if (data1 != 'NA') {
                ...
                if (newData.length > 0) {
                    this.setState({ favourite_arr: newData })
                } else if (newData.length <= 0) {
                    this.setState({ favourite_arr: 'NA' })
                }
            }
        } else {
            this.search.clear();
            this.setState({ favourite_arr: this.state.favourite_arr1, onCross: false });
        }
    }
    cross_click = async () => {
        await this.SearchFilterFunction('');
    ...
    markFavUnfav = async (status, other_id, index) => {
        consolepro.consolelog('markFavUnfav');
        ...
        apifuntion.postApi(url, data).then((obj) => {
            ...
            if (obj.success == "true") {
                ...
                const newList = this.state.favourite_arr.splice(index, 1);
                this.setState({ favourite_arr: this.state.favourite_arr })
            } else {
    ...
    getFavouriteList = async () => {
        consolepro.consolelog('getFavouriteList');
        let user_details = await localStorage.getItemObject('user_arr');
        consolepro.consolelog('user_details_ba', user_details);
        ...
        let url = config.baseURL + "getFavouriteList.php?user_id=" + this.state.user_id;
        consolepro.consolelog(url)
        apifuntion.getApi(url).then((obj) => {
            consolepro.consolelog('obj', obj)
            if (obj.success == "true") {
                this.setState({ favourite_arr: obj.favourite_expert_arr, favourite_arr1: obj.favourite_expert_arr })
                localStorage.setItemObject('favourite_arr', obj.favourite_expert_arr);
                consolepro.consolelog('favourite_arr', this.state.favourite_expert_arr)
            } else {
                msgProvider.alert(msgTitle.information[config.language], obj.msg[config.language], false);
                if (obj.active_status == 'deactivate') {
        ...
        return (
            <View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
                ...
                {this.state.showSearch == false && <View style={styles.setingsHeader}>
                    ...
                    <Text style={styles.OrderHistoryTitle}>Favourite Experts</Text>
                ...
                {this.state.showSearch == true && <View style={{ flexDirection: 'row', backgroundColor: Colors.theme_color, alignItems: 'center', justifyContent: 'space-between', height: windowWidth * 15.2 / 100 }}>
                    <View style={{ width: '86%', backgroundColor: '#F2F2F2', borderColor: 'gray', borderRadius: 40, alignItems: 'center', justifyContent: 'center', marginTop: 10, alignSelf: 'flex-start', marginRight: 10, marginLeft: 25 }}>
                        <TextInput style={{ color: 'black', width: '100%', paddingLeft: 5, textAlign: 'left', paddingRight: 35, height: 38, marginTop: 4, marginLeft: 4 }}
                            placeholder={'Search'}
                ...
                <View style={{ width: mobileW * 100 / 100, paddingBottom: 60 }}>
                    {this.state.favourite_arr == 'NA' && <View style={{ alignItems: 'center', justifyContent: 'center', height: '100%' }}><Image resizeMode="contain" style={{ width: '50%', height: '50%', }} source={localimag.nodataFound}></Image></View>}
                    {this.state.favourite_arr != 'NA' &&
                        <FlatList
                            data={this.state.favourite_arr}
                                    ...
                                    <View style={{ flexDirection: 'row', elevation: 5, backgroundColor: Colors.purewhite, width: windowWidth * 100 / 100, height: windowHeight * 20 / 100, alignItems: 'center', justifyContent: 'flex-start', alignSelf: 'center', marginTop: 8, shadowColor: '#000000', shadowOffset: { width: 2, height: 2, }, shadowOpacity: 0.20 }}>
                                        ...
                                        <View style={{ width: mobileW * 25 / 100, marginRight: 14, height: mobileW * 18 / 100, alignItems: 'flex-end' }}>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15 }}>
                                                ...
                                                />
                                                <Text style={{ color: Colors.theme_color, fontSize: mobileW * 3 / 100 }}>({item.rating == null ? '0' : parseFloat(item.rating).toFixed(1)})</Text>
                                            </View>
                                            <TouchableOpacity activeOpacity={1} onPress={() => { this.markFavUnfav(0, item.user_id, index) }}
                                                style={{ marginTop: mobileW * 5.5 / 100, alignItems: 'center', justifyContent: 'center', marginRight: 20, padding: 10 }}>
                                                <Image resizeMode="contain" style={{ width: mobileW * 4 / 100, height: mobileW * 4 / 100, }} source={localimag.liked}></Image>
                                            </TouchableOpacity>
                                        </View>
                                    </View>
                                )
