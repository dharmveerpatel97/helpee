...
export default class SubscriptionPlan extends Component {
    ...
    takeSubscriptionPlan = async (plan_id, amount) => {
        ...
        this.setState({ plan_id: plan_id, amount: amount, user_id: user_id });
    ...
    updateSignupSteps = async () => {
        ...
        let url = config.baseURL + "updateSignupSteps.php?user_id=" + user_id + "&signup_step=" + '2';
        consolepro.consolelog(url)
        apifuntion.getApi(url).then((obj) => {
            consolepro.consolelog('obj', obj)
            if (obj.success == "true") {
                    ...
                    <View style={{ alignItems: 'center', justifyContent: 'flex-start', }}>
                        ...
                        <View style={{marginBottom:30}}>
                            <TouchableOpacity activeOpacity={1} onPress={() => { this.updateSignupSteps(); }}
                                style={{ borderWidth: 2, borderColor: Colors.theme_color, width: mobileW * 60 / 100, height: mobileH * 7 / 100, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }}>
